...
class GuiClient(QtGui.QWidget):
    ...
    def buttonNameSlot(self):
        if self.senderTurnButton == True and self.turn == "your move":
            self.clickedButton = self.sender()
            self.myGameLoop.senda_message(self.sender().objectName())
        else:
            self.msg_edit.setText("Please wait for your turn!")
    ...
    def connectSlot(self,ms):
        ...
        if newMessage[0] == "new game":
            ...
            self.msg_edit.setText(self.myMessageServer[0])
            if newMessage[1]=="C":
                
                self.role_edit.setText(newMessage[1]+"->You are the Captain")
            else:
                self.role_edit.setText(newMessage[1]+"->You are the General")
                
        elif newMessage[0]== "game over":
            if newMessage[1] == "C":
                self.score_edit.setText('Captain Score: {}\nGeneralScore: {}\nThe Winner is The Captain with a score of: {}'.format(self.captainScore,self.generalScore,self.captainScore))
                
            else:
                self.score_edit.setText('Captain Score: {}\nGeneralScore: {}\n\nThe Winner is The General with a score of: {}'.format(self.captainScore,self.generalScore,self.generalScore))
